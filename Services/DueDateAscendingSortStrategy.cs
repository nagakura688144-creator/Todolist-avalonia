// Services/DueDateAscendingSortStrategy.cs
...
{
    /// <summary>
    /// Sorts TodoItems by due date in ascending order.
    /// Items without due dates are placed at the end.
    /// Items with the same due date are sorted by title.
    /// </summary>
    public class DueDateAscendingSortStrategy : ISortStrategy
    {
        public string Name => "Due Date (Earliest First)";
        ...
        public IEnumerable<TodoItem> Sort(IEnumerable<TodoItem> items)
        {
            ...
            // Concatenate: items with due dates first, then items without
